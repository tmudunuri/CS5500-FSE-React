...
describe('createUser', () => {
    ...
    test('can insert new users with REST API', async () => {
        ...
        const newUser = await createUser(hellen);

        // verify inserted user's properties match parameter user
        expect(newUser.username).toEqual(hellen.username);
        expect(newUser.password).toEqual(hellen.password);
        expect(newUser.email).toEqual(hellen.email);
    });
});
